from Landing_page.models import User, Passions, Assigned_Skills, Business_Experience, Up_For, Collaboration, Colab_Passions, Colab_Assigned_Skills, Colab_Business_Experience, Colab_Up_For, Skills
...
def populate_users(N, first_names, last_names, cities, ratio_match):
	...
	for i in range(0, N):
		...
		if want_to_be_matched==1:
			User.objects.create(email = email, password = passwrd, full_name = first_name+last_name, collaborator = True)
		else:
			User.objects.create(email = email, password = passwrd, full_name = first_name+full_name, ideamaker = True)
...
def populate_mockup_collaboration(cities, colab_data):
	...
	for ind, usr in enumerate(colabs):
		...
		city = cities[random.randint(0,len(cities)-1)]
		Collaboration.objects.create(user = usr, title = title, description = description)
...
def populate():
	...
	rel_data_path = '/usr/src/webserver/Data/'

	skills = read_file(rel_data_path + 'skills_test.txt')
	first_names = read_file(rel_data_path + 'first_names.all.txt')
	last_names = read_file(rel_data_path + 'last_names.all.txt')
	cities = read_csv(rel_data_path + 'world-cities.csv')

	with open(rel_data_path + 'titles.json') as f:
