...
def sort_users_score():
	...
	for colab in colabs:
		...
		print(colab.title, colab.description, ' City:', colab.city)
		print("Prefered profile:")

		print("Passions: ")
		passions = Colab_Passions.objects.filter(colab=colab)
		for passion in passions:
			print(passion.passion.skill)

		print("Assigned_Skills: ")
		assigned_skills = Colab_Assigned_Skills.objects.filter(colab=colab)
		for assigned_skill in assigned_skills:
			print(assigned_skill.assigned_skill.skill)

		print("Business_Experience: ")
		Business_Experiences = Colab_Business_Experience.objects.filter(colab=colab)
		for experience in Business_Experiences:
			print(experience.experience.skill)

		print("Up_For: ")
		Up_Fors = Colab_Up_For.objects.filter(colab=colab)
		for interest in Up_Fors:
			print(interest.interest.skill)
		...
		for match_object in match_objects:
			usr = match_object.user

			print('\n\n',match_object.user, ': ', match_object.score)

			print("Passions: ")
			passions = Passions.objects.filter(user=usr)
			for passion in passions:
				print(passion.passion.skill)

			print("Assigned_Skills: ")
			assigned_skills = Assigned_Skills.objects.filter(user=usr)
			for assigned_skill in assigned_skills:
				print(assigned_skill.assigned_skill.skill)

			print("Business_Experience: ")
			Business_Experiences = Business_Experience.objects.filter(user=usr)
			for experience in Business_Experiences:
				print(experience.experience.skill)

			print("Up_For: ")
			Up_Fors = Up_For.objects.filter(user=usr)
			for interest in Up_Fors:
				print(interest.interest.skill)
				#print(Up_For.interest.skill)
		...
		#	passions = (match_object.user.passions_set.all().skill)

		#	print(passions)
			#skills = passions.skills_set.all().values_list('skill', flat=True)
			#print(skills)
			#Passions.objects.filter(user__first_name = 'bengt')

			#for passion in match_object.user.passions_set.all():
			#	print(passion.passion.skill)
